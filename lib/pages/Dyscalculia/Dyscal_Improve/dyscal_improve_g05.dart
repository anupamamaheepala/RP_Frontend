...
import '/config.dart';
import '/profile.dart';
import 'dyscal_special_task_g05.dart';

class DyscalImproveG05Page extends StatefulWidget {
  const DyscalImproveG05Page({super.key});

  @override
  State<DyscalImproveG05Page> createState() => _DyscalImproveG05PageState();
}

class _DyscalImproveG05PageState extends State<DyscalImproveG05Page> {
  bool _isLoading = true;
  String _currentLevel = "easy";
  int _tasksCompleted = 0;
  List<dynamic> _questions = [];

  // Track whether this session just finished a task set
  ...
  Future<void> _initLearningPath() async {
    ...
    try {
      ...
      final stateUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-state/$userId/5");
      ...
      if (stateRes.statusCode == 200) {
        ...
        // Only show special task dialog if triggered by task completion
  ...
  Future<void> _fetchQuestions() async {
    try {
      final qUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-questions/5/$_currentLevel");
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(builder: (context) => const DyscalSpecialTaskG05Page())
              );
            },
          ...
          child: Column(
            children: [
              AppBar(
                ...
                title: const Text('දියුණු කිරීම - 5 ශ්‍රේණිය', style: TextStyle(color: Colors.purple, fontWeight: FontWeight.bold)),
                centerTitle: true,
              ),
              Expanded(
                child: _isLoading
