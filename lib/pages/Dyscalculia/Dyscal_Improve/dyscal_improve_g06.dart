...
import '/config.dart';
import '/profile.dart';
import 'dyscal_special_task_g06.dart';

class DyscalImproveG06Page extends StatefulWidget {
  const DyscalImproveG06Page({super.key});

  @override
  State<DyscalImproveG06Page> createState() => _DyscalImproveG06PageState();
}

class _DyscalImproveG06PageState extends State<DyscalImproveG06Page> {
  ...
  Future<void> _initLearningPath() async {
    ...
    try {
      ...
      final stateUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-state/$userId/6");
  ...
  Future<void> _fetchQuestions() async {
    try {
      final qUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-questions/6/$_currentLevel");
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(builder: (context) => const DyscalSpecialTaskG06Page())
              );
            },
          ...
          child: Column(
            children: [
              AppBar(
                ...
                title: const Text('දියුණු කිරීම - 6 ශ්‍රේණිය', style: TextStyle(color: Colors.purple, fontWeight: FontWeight.bold)),
                centerTitle: true,
              ),
              Expanded(
                child: _isLoading
