...
import '/config.dart';
import '/profile.dart';
import 'dyscal_special_task_g07.dart';

class DyscalImproveG07Page extends StatefulWidget {
  const DyscalImproveG07Page({super.key});

  @override
  State<DyscalImproveG07Page> createState() => _DyscalImproveG07PageState();
}

class _DyscalImproveG07PageState extends State<DyscalImproveG07Page> {
  ...
  Future<void> _initLearningPath() async {
    ...
    try {
      ...
      final stateUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-state/$userId/7");
  ...
  Future<void> _fetchQuestions() async {
    try {
      final qUrl = Uri.parse("${Config.baseUrl}/dyscalculia/learning-questions/7/$_currentLevel");
          ...
          ElevatedButton(
            ...
            onPressed: () {
              ...
              Navigator.pushReplacement(
                  context,
                  MaterialPageRoute(builder: (context) => const DyscalSpecialTaskG07Page())
              );
            },
          ...
          child: Column(
            children: [
              AppBar(
                ...
                title: const Text('දියුණු කිරීම - 7 ශ්‍රේණිය', style: TextStyle(color: Colors.purple, fontWeight: FontWeight.bold)),
                centerTitle: true,
              ),
              Expanded(
                child: _isLoading
